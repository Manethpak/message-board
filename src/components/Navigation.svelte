...
<nav class="hidden md:block">
  <div class="flex gap-10 justify-center">
    ...
    {:else}
    <a href="/profile" use:link class="text-xl hover:text-gray-300">Hi, {$userStore.displayName}!</a>
...
<nav class="block md:hidden">
  ...
  <ul transition:slide|local class="bg-neutral-600 w-full flex flex-col gap-4 py-2 px-4 mt-2 rounded-md">
    ...
    <a href="/about-us" use:link class=" hover:text-gray-300">About Us</a>
    
    {#if $userStore === null}
    <a href="/login" use:link class=" hover:text-gray-300">Login</a>
    <a href="/register" use:link class=" hover:text-gray-300">Register</a>
    {:else}
    <a href="/profile" use:link class="hover:text-gray-300">Hi, {$userStore.displayName}!</a>
